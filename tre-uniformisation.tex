...
\begin{itemize}
	\item$\begin{aligned}[t]
			      \delta'' : (\parts Q \times \parts {Q'})^2 \times \Sigma & \to      \parts Q \times \parts {Q'}                                                      \\
			      ((X,X'),(Y,Y'),a)                                        & \mapsto (\{ \delta(x,y,(a,b)) \} \mid  \{x \in X, y \in Y, b \in \set{0,1}\}, f(X',Y',a))
		      \end{aligned}$
...
\begin{lemma}
    If $A'$ is "universal" then $\phi$ is "uniformisable".
\end{lemma}

\begin{proof}
    We need to exhibit a $\psi$ that is a "uniformiser" for $\phi$.
\end{proof}
