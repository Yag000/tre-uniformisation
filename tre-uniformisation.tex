...
nondeterministic bottom‐up tree automaton whose transition relation is \emph{order‐insensitive} (\ie closed under sibling swaps). We then perform a universality check on this
...
\begin{remark}
	A "DBUA" is a particular case of a "NBUA" , $A = (\Sigma, Q, I, \Delta, F)$ verifying that
	\[ \text{for all } q,r,a,t,t' \ (q,r,a,t) \in \Delta \text{ and } (q,r,a,t') \in \Delta  \text{ implies } t = t' \]
\end{remark}
...
We aim now to show that, like in the classical theory, both non-deterministic automata and deterministic ones have the same expressiveness power. Since "DBUA" is
a particular case of "NBUA" it suffices to show that each language accepted by an "NBUA" can also be accepted by a "DBUA".
