...
\newcommand{\papertitle}{Decidable MSO Uniformisation on Binary Trees via Order-Insensitive Automata}
...
\begin{lemma}
	Let $A_1 = (\Sigma, Q_1, I_1, \Delta_1, F_1)$ and $A_1 = (\Sigma, Q_2, I_2, \Delta_2, F_2)$ be two "order-insensitive" "NBUA", then
    ...
    is "order-insensitive" and verifies that $\lang {A_3} = \lang {A_1} \cap \lang {A_2}$
\end{lemma}

\begin{lemma}
	Let $A_1 = (\Sigma, Q_1, I_1, \Delta_1, F_1)$ and $A_1 = (\Sigma, Q_2, I_2, \Delta_2, F_2)$ be two "order-insensitive" "NBUA", then
	\[ A_3 = (\Sigma, Q_1 \uplus Q_2, I_1 \uplus I_2, \Delta_1 \uplus \Delta_2, F_1 \uplus F_2) \]
    is "order-insensitive" and verifies that $\lang {A_3} = \lang {A_1} \cup \lang {A_2}$
\end{lemma}

\begin{lemma}
	Let $A = \NBUA$ the "order-insensitive" "NBUA" $A' = (\Sigma, Q, I, \Delta, Q \cap F)$
	verifies that $\lang {A'} = (\lang A )^{\complement}$
\end{lemma}

\begin{lemma}
	Over the alphabet $\Sigma \times \set {0,1}$ the "order-insensitive" automaton $A$ described bellow can decide the formula $a (x)$,
...
\begin{lemma}
	Over the alphabet $\Sigma \times \set {0,1}^2$ the "order-insensitive" automaton $A$
    described bellow can decide the formula $x  \ancestor y $, where $x$ and $y$ are free variables.
...
\begin{proof}
	...
	We have to deal with the case $\phi = \exists x.\psi(x)$, where $\psi$ has $n-1$ free variables. We can find
    an "order-insensitive" "NBUA" $A = (\Sigma \times \set{0,1}^n, Q, I, \Delta, F)$ for $\psi$ where $x$ is treated as a free variable.
	...
	\[
		...
		\end{aligned}
	\]
	and $i$ corresponds to the variable $x$, is "order-insensitive" and can decide $\phi$. If existential quantification is applied to a
