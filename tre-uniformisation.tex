...
\todo{make this work with new sections}
...
\Cref{sec:trees} introduces our notation for trees.
\Cref{sec:automata} reviews bottom‐up tree automata, \Cref{sec:OrderAutomata} presents the order‐insensitive variant,
and \Cref{sec:UniversalityAutomata} defines the universality test.
...
\todo{write intro here}

\begin{definition}
	\AP Let $A = \DBUA$ a "DBUA", let $\intro* \Reach A \subseteq Q$ be the smallest set such that
	\begin{enumerate}
...
\begin{definition}
	\AP Let $A $ a "NBUA" we say that it is ""universal"" if it "accepts" every "tree".
\end{definition}
...
The main result is \todo{make reference}.

\subsection{Definitions}

We are going to introduce some basic notions on this subject.

\begin{definition}[Uniformiser]\label{def:uniformiser}
	\AP We say that a formula $\psi(X)$ is a ""uniformiser"" of $\phi(X)$ if for all "trees" $t$, the following conditions hold:
	\begin{enumerate}
	...
	A formula is ""uniformisable"" if there exists a "uniformiser" for said formula.
\end{definition}

Another way of describing this property is to say that a formula $\phi(X)$ is "uniformisable" if there exists a formula $\psi(X)$
such that for every "tree" $t$, if there exists $X$ such that, $t \models \phi(X)$ then $t \models \phi(\setdef x {t \models \psi x})$.
This property is a global property on the formula, the "uniformiser" we find must work for every tree. These two definitions are equivalent.
There exists a local version of this property called "definability".
\begin{definition}
	\AP Let $t$ be a "tree". We say that $X \subseteq \Nodes t$ is ""definable"" if there exists and "MSO" formula $\phi(x)$ such that
	$ X = \setdef x {t \models \phi(x)}$.
\end{definition}
In particular, if $\phi(X)$ is "uniformisable" then for every "tree", if $\phi(X)$ is satisfiable there exists a $X_d$ "definable" that also satisfies $\phi$.

"Definiability" expresses the idea that we can construct, using "MSO", a solution for a particular "tree", while "uniformisability" expresses the idea
of being able to find a formula that constructs a particular solution for every "tree".

\subsection{The main result}

What we are after in this paper is an algorithm capable of deciding if a given formula has a "uniformiser" and that is able to give us said "uniformiser".
Our technique consist on studying a similar problem to uniformisation, instead fo trying to find a "uniformiser" we will try to find a
definable solution for each "tree". So we will study the following problem

\begin{problem}\label{prob:def}
Given a formula $\phi(X)$, can we find for every "tree"$t$ a "definable" $X_d$ such that if $t \models \exists X. \phi(X)$ then
$t \models \phi(X_d)$.
\end{problem}



In an intuitive way, \Cref{prob:def} is equivalent to asking whether,
...
\begin{definition}
	\AP Let $\phi(X)$ a MSO formula, and $A =  (\Sigma \times \{0,1\}, Q, \init, \delta, F)$ the "DBUA" for $\phi(X)$, we call
	$U = (\Sigma, \parts Q \times \parts Q,\init_U, \deltau, F')$ the ""uniformiser automaton"" of $\phi$, where
	\begin{itemize}
		\item$\begin{aligned}[t]
				      \init_U : \Sigma & \to      \parts Q \times \parts {Q} \\
				      a                & \mapsto  ( \init(a,*), \init(a,*))
			      \end{aligned}$
		      \itemAP $\begin{aligned}[t]
...
\begin{lemma}
	Let $\phi(X)$ be an "MSO" formula and $A$ it's corresponding automaton. The following statements are equivalent:
	\begin{enumerate}
		\item $\phi(X)$ is "uniformisable".
		\item For all "tree" $t$ there exists a "definable" $X_d$ such that if $t \models \exists X. \phi(X)$ then $t \models \phi(X_d)$.
		\item For all "tree" $t$ if $t \models \exists X. \phi(X)$ then there exists an $X$ such that $X = \sigma (X)$ for all
		      $\sigma \in \Aut(t)$ and $t \models \phi(X)$.
		\item The "unifier automata" of $\phi$ is "universal".
	\end{enumerate}
\end{lemma}
