...
\begin{proof}
	...
	We consider $U = (\Sigma, \parts Q \times \parts Q,\init', \delta', F')$ then "unifier automaton" of $\phi$.
	...
	\begin{eqnarray} \psi &=& \exists X_0, \ldots, X_{k-1} \quad \forall x \left( \bigvee_{i=0}^{k-1} X_i(x) \right) \\
		...
		&& \land \forall x \left( \leaf (x) \rightarrow \bigvee_{(a,i)\in I} X_i(x) \right) \\
		&& \land \forall x \forall y \forall z \Big( \children(x,y,z) \rightarrow \\
		&& \quad \quad \bigvee_{(X',Y',a,Z')\in \delta} \bigvee_{\substack{p \in X' \\ q \in Y' \\ b \in \{0,1\} \\ r \in Z'}} \big( \choice(X',Y',Z',a) = (p,q,b) \\
		&& \quad \quad \quad \land X_p(x) \land X_q(y) \land a(z) \land X_r(z) \big) \Big) \\
