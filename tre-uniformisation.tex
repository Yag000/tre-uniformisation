...
\begin{proof}
	...
	We define the set $X_U$ and the "run" $\intro*\rhou$ over the annotated "tree" $(t,X_U)$ by induction on the depth of the node as follows:
	...
	\begin{itemize}
		...
		\item For the inner nodes $x$ with children $y$ and $z$, there exists $r \in Q$, such that $\rhou (x) = r$ by the induction hypothesis. Then,
		      ...
		      of $A$ over $(t,X_U)$
		\item For the "leaves" $x$ of "label" $a$, by the induction hypothesis we have that $b = \choiceL(a, \rhou(x))$ which is well defined since $a \in \Sigma$ and unique.
	...
	Given a set of "states" $P$, from \zcref{lem:MSO-aut} there exists a formula $\intro* \ustate_P(x)$ such that
	\todo{fix ustate ref}
	...
	\item The last thing we have to prove is that there is a unique $\rhou, X$ that satisfies the formula.
	\todo{~}
\end{proof}
