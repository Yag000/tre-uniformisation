...
\begin{proof}
	...
	We have to deal with the case $\phi = \exists x.\psi(x)$. We can find a "NBUA" $A = \NBUA$ for $\psi$ where $x$ is treated as a free variable.

	Then the automaton $A' = (\Sigma \times \set{0,1}, Q \times \set{0,1}, I \times \set 0, \Delta', F \times \set 1)$
	where
	...
	\[
		\begin{aligned}
			...
			\begin{aligned}
				 ...
				 & \quad u'' = u \land u', X \text{ has } 0 \text{ at position } i, \\
			...
			\begin{aligned}
				 & ((q,0), (q', 0), (a, X), (q'',1)) \mid          \\
				 & \quad X3 \text{ has } 1 \text{ at position } i, \\
				 & \quad (q, q', (a, X), q'') \in \Delta
			\end{aligned}
			\right\}.
		\end{aligned}
...
\begin{problem}
Given an MSO formula $\phi$, determine whether there exists an MSO formula $\psi$ such that, for all "trees" $t$, the following conditions hold:
\begin{enumerate}
	\item There exists at most one element $x$ such that $\Model t \models \psi(x)$.
	\item If there exists an element $x$ such that $\Model t \models \phi(x)$, then there exists an element $y$ such that $\Model t \models \psi(y) \land \phi(y)$.
\end{enumerate}
\end{problem}
...
\begin{problem}
Given an MSO formula $\phi$, decide whether there exists an MSO formula $\psi$ such that, for all "trees" $t$, the following holds:
\begin{eqnarray*}
...
\begin{proof}
	...
	\begin{enumerate}
		\item The formula $\forall X \forall Y \, (\psi(X) \land \psi(Y)) \rightarrow X = Y)$ ensures that $\psi$ is satisfied by at most one element
		...
		\item The formula $\exists X \, \phi(X) \rightarrow \exists X \, \psi(X)$ expresses that whenever $\phi$ has a solution, $\psi$ must also have a solution.
		      However, this does not require $\psi$ to hold for the same element as $\phi$.

		      The formula $\forall X \, (\psi(X) \rightarrow \phi(X))$ ensures that whenever $\psi$ holds for an element, $\phi$ must also hold for that same element.

		      Combining the last two constraints, we obtain the second condition: whenever there exists an element satisfying $\phi$, there must exist an element
