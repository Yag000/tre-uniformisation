...
\begin{proof}
	...
	Here is the corrected version with spelling and minor grammatical fixes:

	We define the following formulas:
	\begin{itemize}
		\item This formula encodes the fact that \( x \) is a "leaf":
		      \[\text{leaf} (x) = \forall y, \ancestor y x \]
		\item This formula encodes the fact that \( x \) is the root of the tree:
		      \[\text{root} (x) = \forall y, \ancestor x y \]
		\item This formula encodes the fact that \( x \) and \( y \) are the immediate children of \( z \):
		      \[\text{children} (x,y,z) =
			      x \neq y \land
	...
	\begin{eqnarray*}
		...
		&\land& \forall x \left( \bigwedge_{0 \leq i \leq j \leq k-1}  \lnot (X_i(x) \land X_j(x)) \right) \\
		&\land& \forall x  \left( \text{leaf} (x) \ra \bigvee_{(a,i)\in I}  X_i(x) \right) \\
		&\land& \forall x \forall y \forall z \left(\text{children}(x,y,z) \ra \bigvee_{(i,j,a,k)\in \Delta} \left(X_i(x) \land X_j(y) \land a(z) \land X_k(z)\right)\right) \\
		&\land& \forall x \left(\text{root}(x) \ra \bigwedge_{i= 0}^k \left( X_i(x) \ra \bigvee_{a \in F_i} a(x) \right)\right)
	\end{eqnarray*}
