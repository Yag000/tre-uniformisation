...
\begin{proof}
	...
	We define the set $X$ ant the "run" $\intro*\rhou$ over the "tree" $(t,X)$ by induction on the depth of the node as follows:
	\begin{itemize}
		\item For the root, we define $\rhou(\varepsilon) = \choice(\delta'(t))$.
		\item For all inner nodes $x$ with children $y$ and $z$. Let $(q,r,v)$ be~$\choice(\deltau(\subtree t y), \deltau(\subtree t z), \rhou(x), \tlabel t x)$, we define
		$\rhou(y)=q$, $\rhou(z)=r$, and $x \in X$ if and only if $b = 1$.
		\item For all "leaves" $x$, we define $(\rhou(x), b) = \choice(\tlabel tx)$ where $x \in X$ if and only if $b = 1$.
	\end{itemize}

	We claim now that the $\rhou$ is well defined, unique, and indeed a "run".
	For this, we show by top-down induction that for all "nodes" $x$ of $t$, there exists a unique "state" $q$ such that $\rhou(x) = q$.
	\begin{itemize}
	...
	We claim also that for all "nodes" $x$ of $t$, $\rhou(x) \in \deltau(\subtree t x)$ and that $\rhou(X)$ is a "run" of $A$ over $(t,X)$.
		 ...
		 This formula simply states using the MSO-syntax the existence of the run $\rhou$ described above, ie.
		      \[  \Model t \models \unif(X) \text{ \tiff } X=X_U% \text{ corresponds to the set ''computed'' by } \rhou.
		      \]
