...
\begin{theorem}
	...
	\]
    and $ \lang N = \lang {\Det N}. $
\end{theorem}

\begin{proof}
	We will first show that for all "tree" $t$,
	\[
		\interpret  t = \setdef {\rho(\epsilon)}{\text{$\rho$ "run" of~$N$ over~$t$}}\ .
	\]
	...
	\begin{itemize}
		...
		\item $(t = a(t_1,t_2))$\\
		      ...
		      \begin{eqnarray*}
			      &&q \in \interpret D t \\
			      &\text{if and only if}& q \in \delta_a (\interpret D {t_1}, \interpret D {t_2}) \\
			      &\text{if and only if}& q \in \setdef {q \in  Q} {(x_1,x_2,a,q) \in \Delta, x_1 \in \interpret D {t_1}, x_2 \in \interpret D {t_2}} \\
