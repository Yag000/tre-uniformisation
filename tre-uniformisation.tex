...
\begin{proof}
	...
	If $\phi$ is of the form $\psi_1 \land \psi_2$ then we can compute the intersection between the automata for $\psi_1$ and $\psi_2$, obtained using the induction hypothesis.
	We can deal with the disjuction using the union instead of the intersection.
	...
	\[
		...
		\end{aligned}
	\]
	and $i$ corresponds to the variable $x$, can decide $\phi$. If the existential quantification is performed on a
	variable of the second order we check the automation remains the same but instead of looking at a single variable
	in the transition function we look at all of the ones present in the set.
