...
\begin{proof}
	...
	Then the automaton $A' = (\Sigma \times \set{0,1}^{n-1}, Q \times \set{0,1}, I \times \set 0, \Delta', F \times \set 1)$
	where
	\[
		\begin{aligned}
			\Delta' = & \left\{
...
\begin{problem}
Given an MSO formula $\phi(x)$, determine whether there exists an MSO formula $\psi(x)$ such that, for all "trees" $t$, the following conditions hold:
\begin{enumerate}
...
\begin{problem}
Given an MSO formula $\phi(X)$, decide whether there exists an MSO formula $\psi(X)$ such that, for all "trees" $t$, the following holds:
\begin{eqnarray*}
