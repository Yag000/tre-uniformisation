...
Our main result, \Cref{theorem:main}, is that this question is \emph{decidable}, and in the positive case, a "uniformiser" $\psi$ can be effectively constructed. Concretely, we translate $\phi(X)$ into a
...
\todo{unofrmisation problem is not really defined}
In \Cref{sec:uniformisation}, we define the "uniformisation problem" and present our main result, showing that it is decidable for "unordered finite binary trees".
\todo{talk about knowledge, see Thomas' paper}
...
\todo{Tak about the main result (MSO aut) at some point, and do it better than right now}
