...
In \Cref{sec:uniformisation}, we define "uniformisation" and present our main result, showing that it is decidable for "unordered finite binary trees".
...
While our definition of "trees" is ordered, we can syntactically distinguish between left and right "children" of a "node",
...
The proof follows the classical schema \cite{Buchi60, Thomas1997, bookautomata}, but restricting to "order-insensitive" "NBUA" and "MSO formulas"

We start by showing that every "automaton@NBUA" can be expressed as a formula. We will encode the existence of a valid "run" of the "automaton@NBUA" 
...
\begin{proof}
    ...
    Since all of this proportier encode the existence of a valid "run" of the automaton $A$ over the "tree" $t$, we have that 
...
\begin{lemma}\label{lem:atomic-a}
	...
	This automaton decides the "atomic predicate" $a(x)$, where $x$ is a free first-order variable.
\end{lemma}
...
\begin{lemma}\label{lem:atomic-ancestor}
	...
	This automaton decides the "atomic predicate" $x \ancestor y$, where $x$ and $y$ are free first-order variables.
\end{lemma}
