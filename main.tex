...
\begin{lemma}\label{lem:main-result}\label{lemma:main}
	...
	\begin{enumerate}
		...
		\item For all "trees" $t$ that models $\phi(X)$ for some~$X$, $t$ models~$\phi(X)$ for some "definable"~$X$. \label{lem:main-result-2}
		      % invariant under the automorphisms of~$t$ (ie  $\sigma(X) = X$ for all $\sigma \in \Aut(t)$). \label{lem:main-result-3}
		\item For all "trees" $t$ that models $\phi(X)$ for some~$X$, $t$ models~$\phi(X)$ for some~$X$ invariant under the automorphisms of~$t$ (ie such that $\sigma(X) = X$ for all $\sigma \in \Aut(t)$). \label{lem:main-result-3}
		      %\item For all "trees" $t$ such that $t \models \exists X. \phi(X)$, there exists $X$ such that $t \models \phi(X)$ and $\sigma(X) = X$ for all $\sigma \in \Aut(t)$. \label{lem:main-result-3}
...
\begin{definition}["Uniformiser automaton"]
	...
	\end{itemize}
	\AP Using the notation $\delta(X,Y,(a,""*""))$ for $\bigcup_{b \in \set{0,1}} \delta(X,Y,(a,b))$, and $\init(a,\reintro{*})$ for $\bigcup_{b \in \set{0,1}} \init(a,b)$.
\end{definition}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item For the inner nodes $x$ with children $y$ and $z$, there exists $r \in Q$, such that $\rhou (x) = r$ by the induction hypothesis. Then,
		      ...
		      We have that $\rhou(y) = p$ and $\rhou(z) = q$, which verify that $\delta(p,q,(\tlabel t x,b)) = r$. We also have that $x \in X_U$ if and only if $b = 1$.
...
\begin{proof} \todo{change : to $\mid$} \todo{knoledge all *}
	...
	\begin{enumerate}
		\item $R \subseteq \{ \delta_A (t_{R,R'},X) \mid X \subseteq \Nodes {t_{R,R'}}\}$ \label{lem:prop1}
		\item $\{ \delta_A (t_{R,R'},X) \mid \forall \sigma \in \Aut(t_{R,R'}), \sigma(X) = X\} \subseteq R'$\label{lem:prop2}
	...
	If $(T,T') = \delta_{\Ua}((P,P'),(Q,Q'),a)$ for $a \in \Sigma$ and $P,P',Q,Q' \in \Reach \Ua$, then by the induction hypothesis there exists
	...
	$\interpret \Ua {t_{T,T'}} = \delta_{\Ua} (\interpret \Ua {t_{P,P'}}, \interpret \Ua {t_{Q,Q'}}, a)$.
	If we use \Cref{lem:prop3} of the induction hypothesis we deduce that
	$\delta_{\Ua} (\interpret \Ua {t_{P,P'}}, \interpret \Ua {t_{Q,Q'}}, a) = \delta_{\Ua} ((P,P'), (Q,Q'), a)$
	And by definition of $(T,T')$, $\delta_{\Ua} ((P,P'), (Q,Q'), a) = (T,T')$.
	\Cref{lem:prop3} is then verified.

	By definition, $T = \delta_A(P,Q,(a,"*"))$. Since by \Cref{lem:prop1} of the induction hypothesis $P \subseteq \{ \delta_A (t_{P,P'},X) \mid X \subseteq \Nodes {t_{P,P'}}\}$
	and $Q \subseteq \{ \delta_A (t_{Q,Q'},X) \mid X \subseteq \Nodes {t_{Q,Q'}}\}$  we have that
	$T = \delta_A(P,Q,(a,"*")) \subseteq \{ \delta_A (t_{T,T'},X) \mid X \subseteq \Nodes {t_{T,T'}}\}$. We have proven \Cref{lem:prop1}.
	...
	\begin{itemize}
		...
		\item Lets suppose $t_{P,P'} \ncong t_{Q,Q'}$, in this case we have that $P' \neq Q'$ or $P \neq Q$ and thus $T' = \deltauu (P',Q',a) = \delta_a(P',Q',(a,"*"))$.
		      ...
		      Let $p := \delta_A (t_{P,P'}, X_1)$ and $q := \delta_A (t_{Q,Q'}, X_2)$.
	...
	$X \in \{ \delta_A (t_{R,R'},X) \mid X \subseteq \Nodes {t_{R,R'}}\} \cap F$.
	We also know that $R' \cap F = \emptyset$, we can the deduce that $\{ \delta_A (t_{R,R'},X) \mid \forall \sigma \in \Aut(t_{R,R'}), \sigma(X) = X\} \cap F = \emptyset$
	thanks to \Cref{lem:prop2}. So we know that $X \notin \{ \delta_A (t_{R,R'},X) \mid \forall \sigma \in \Aut(t_{R,R'}), \sigma(X) = X\} \cap F = \emptyset$ which means that no
