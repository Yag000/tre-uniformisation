...
\Cref{lem:main-result-1} to \Cref{lem:main-result-2}, and from \Cref{lem:main-result-2} to \Cref{lem:main-result-3}. The "uniformiser automaton" is then defined in the next section, \kCref{uniformiser automaton}. The implication from \Cref{lem:main-result-3} to \Cref{lem:main-result-4} is then the subject of \Cref{lemma:...} in \Cref{section:...}, and the implication from~\Cref{lem:main-result-4} to \Cref{lem:main-result-1} is stated in \Cref{lemma:...}, \Cref{section:...}.
...
there exists a "definable" $X_d$ such that $t \models \phi(X_d)$. ... This proves the implication from \Cref{lem:main-result-1} to \Cref{lem:main-result-2} in \Cref{lemma:main}.
...
\begin{example}\todo{remove color and join with remark}
...
then $\phi(X)$ is "uniformisable", and in \Cref{lemma:non-universal-implies-non-invariant} that if it is not "universal", then there exists a 
...
\begin{lemma}\label{lemma:non-universal-implies-non-invariant}
...
\begin{proof}
	...
	\begin{enumerate}
        ...
        \item $\{ \delta_A (t_{R,R'},X) : \forall \sigma \in \Aut(t_{R,R'}), \sigma(X) = X\} \subseteq R'\}$\label{lem:prop2}
	...
	We also know that $R' \cap F = \emptyset$ we can the deduce that $\{ \delta_A (t_{R,R'},X) : \forall \sigma \in \Aut(t_{R,R'}), \sigma(X) = X\} \subseteq R'\} \cap F = \emptyset$
	thanks to \ref{lem:prop2}. So we know that $X \notin \{ \delta_A (t_{R,R'},X) : \forall \sigma \in \Aut(t_{R,R'}), \sigma(X) = X\} \subseteq R'\} \cap F = \emptyset$ which means that no
